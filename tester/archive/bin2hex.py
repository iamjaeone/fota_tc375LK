"""
File: bin2hex.py (Python 3 compatible)
...
# ... (HexRecord and other functions remain the same as they are mostly compatible) ...
    ...
    for (addr, binfile) in inputlist:
        with open(binfile, 'rb') as f:
            ...
            while f.tell() != fsize:
                ...
                # In Python 3, reading a binary file returns a 'bytes' object, which is already a sequence of integers.
                # The `map(ord, ...)` line from Python 2 is no longer needed and will cause an error.
                # We can directly use the 'bindata' bytes object.
                hexout.append(HexRecord(HEX_TYPE_DATA, list(bindata), address=curraddr).getRecord())
...
def commandline_split(option, opt, value, parser):
    ...
    # *** MODIFICATION FOR PYTHON 3 ***
    # In Python 3, map() and zip() return iterators, so we convert them to a list explicitly.
    binlist = list(map(list, zip(*[iter(binlist)]*2)))
    binlistout = []
...
if __name__ == "__main__":
    ...
    if settings.version:
        # *** MODIFICATION FOR PYTHON 3 ***
        print("bin2hex.py %s"%("0.1 (Python 3 compatible)"))
        sys.exit(0)
